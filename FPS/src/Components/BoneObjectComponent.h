...
struct BoneObjectComponent {
	...
	BoneObjectComponent(const string& Filename) {
		filename = Filename;
		m_VAO = 0;
		ZERO_MEM(m_Buffers);
		m_NumBones = 0;
	...
	bool LoadBoneObjectComponent(const string& Filename)
	{
	...
	bool InitFromScene(const aiScene* pScene, const string& Filename)
	{
		...
		for (uint i = 0; i < m_Entries.size(); i++) {
			...
			InitBoneObjectComponent(i, paiMesh, Positions, Normals, TexCoords, Bones, Indices);
		}
		...
		for (uint i = 0; i < pScene->mNumMaterials; i++) {
			...
			if (pMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
				...
				if (pMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
					...
					m_Textures[i] = new Texture(Load(FullPath.c_str()));
	...
	void InitBoneObjectComponent(uint MeshIndex,
